readDataIndels_forgenes <- function(vcf,path,populations=FALSE,outgroup=FALSE,include.unknown=FALSE,gffpath=FALSE,format="fasta",parallized=FALSE,progress_bar_switch=TRUE,
...
for(i in 1:length(matrixCCC)){
        idx <- which(matrixCCC[i]=="t"|matrixCCC[i]=="T")
        matrixCCC[idx,i] <- 1
        idx2 <- which(matrixCCC[i]=="c"|matrixCCC[i]=="C")
        matrixCCC[idx2,i] <- 2
        idx3 <- which(matrixCCC[i]=="g"|matrixCCC[i]=="G")
        matrixCCC[idx3,i] <- 3
        idx4 <- which(matrixCCC[i]=="a"|matrixCCC[i]=="A")
        matrixCCC[idx4,i] <- 4
        idx5 <- which(matrixCCC[i]=="n"|matrixCCC[i]=="N")
        matrixCCC[idx5,i] <- 5
        idx6 <- which(matrixCCC[i]=="-")
        matrixCCC[idx6,i] <- 6
        idx7 <- which(matrixCCC[i]=="I"|matrixCCC[i]=="i")
        matrixCCC[idx7,i] <- 7
}

matrixCCC <- apply(matrixCCC,2,as.numeric)
matrixCCC <- t(matrixCCC)
