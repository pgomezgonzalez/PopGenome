get_data_ins <- function(matr,include.unknown=FALSE,gff=FALSE,FAST,SNP.DATA){
...
matrix_pol <- integer(nsamtot)
      ...
      pyrimid         <- c(1,2)  #pyrimid is deletion
      purin           <- c(3,4)  #purin is insertion
      err             <- c(5,6)
      ins             <- c(7)
...
erg <- apply(matr,2,function(check){
      ...
      fuenfsechs   <- is.element(err,check) # err = 5,6
      sietes       <- is.element(ins,check)
